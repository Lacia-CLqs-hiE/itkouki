...
if( !empty($_POST['btn_submit']) ) {
    ...
    if( empty($error_message) ) {
        ...
        try {
        //データベースに接続
        $dbh = new PDO('mysqli:host=localhost;dbname=board', 'root', 'arx923m3v12');
        
        //接続エラーの確認
        if( $dbh->connect_errno ){
            $error_message[] = '書き込みに失敗しました。 エラー番号 '.$dbh->connect_errno.' : '.$dbh->connect_error;
        } else {
            //文字コードの設定
...
//接続エラーの確認
if( $dbh->connect_errno ){
    $error_message[] = 'データの読み込みに失敗しました。 エラー番号 '.$dbh->connect_errno.' : '.$dbh->connect_errno;
} else {
